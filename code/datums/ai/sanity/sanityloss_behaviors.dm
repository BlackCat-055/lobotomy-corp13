		...
		if(target_turf && get_dist(living_pawn, target_turf) < 3)
			if(!step_towards(living_pawn, target_turf)) //If it fails to move
				...
				if(get_turf(living_pawn) == target_turf)
					...
					controller.pathing_attempts++
			var/move_delay = max(0.8, 0.2 + living_pawn.cached_multiplicative_slowdown - (get_modified_attribute_level(living_pawn, DEXTERITY_ATTRIBUTE) * 0.004))
		...
		if(target_turf && get_dist(living_pawn, target_turf) < 3)
			...
			if(!step_towards(living_pawn, target_turf)) //If it fails to move
				...
				if(get_turf(living_pawn) == target_turf)
					...
					controller.pathing_attempts++
			var/move_delay = max(0.8, 0.2 + living_pawn.cached_multiplicative_slowdown - (get_modified_attribute_level(living_pawn, DEXTERITY_ATTRIBUTE) * 0.002))
